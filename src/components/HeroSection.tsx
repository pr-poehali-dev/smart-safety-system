  ...
  return (
    <section className="flex flex-col items-center justify-center px-4 pt-16 pb-10 text-center">
      <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-[#1a1a1a] py-2 text-sm px-3">
        ...
        <span className="text-gray-300">Переводы за 3 секунды — уже доступно</span>
        <ArrowUpRight className="h-4 w-4 text-gray-400" />
      </div>

      <h1 className="mb-4 max-w-3xl text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-white text-balance">
        Финансы вашего бизнеса — в одном потоке
      </h1>

      <p className="mb-8 max-w-xl text-gray-400 text-lg leading-relaxed">
        Переводы, контроль доступа, синхронизация счетов — без лишних инструментов и задержек.
      </p>

      <div className="flex flex-col sm:flex-row items-center gap-4">
        <Button className="rounded-full bg-violet-600 px-7 py-5 hover:bg-violet-700 text-white text-base">
          Начать бесплатно <ArrowUpRight className="ml-2 h-4 w-4" />
        </Button>
        <Button variant="outline" className="rounded-full border-gray-700 bg-transparent text-white hover:bg-gray-800 px-7 py-5 text-base">
          <Play className="mr-2 h-4 w-4 fill-violet-500 text-violet-500" /> Смотреть демо
        </Button>
      </div>

      <p className="mt-5 text-xs text-gray-600">Без карты. Первые 14 дней бесплатно.</p>
    </section>
  )
}